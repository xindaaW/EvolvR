...
pe = []
file_path = "/home/deepspeed/workdir/LLaMA-Factory-25049/HANNA/Pair/train.json"
...
for sample in data:
    ...
    prompt1 = prompts_fewshot.format(
        ...
        prompt1=pe1, context2=context2, score2=score2, prompt2=pe2
    )
    prompt2 = prompts_fewshot.format(
        aspect=aspect, context1=context2, score1=score2, 
        prompt1=pe2, context2=context1, score2=score1, prompt2=pe1
    )
...
llm = LLM(
    model="/home/deepspeed/workdir/Qwen2.5-7B-Instruct", 
...
for item in data:
    output1 = outputs[output_index]
    output2 = outputs[output_index + 1]
    output_index += 2
    ...
    if saved_count == 0:
        skipped_count += 1
        print(f"Warning: sample {output_index//2-1} has both incomplete results, skipped.")
    elif saved_count == 1:
        print(f"Hint: sample {output_index//2-1} has only one complete result, retained.")

output_path = "../Pairwise_7B_CoT_hanna.json"
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)
    
print(f"Saved {len(result)} valid results, completely skipped {skipped_count} samples")
