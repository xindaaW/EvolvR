...
instruction="""
    ...
    # Background:
    The user needs to evaluate a given story segment from the {aspect} perspective, considering the comments provided.
    ...
    # Skills:
    You possess multiple capabilities including story creation, story structure analysis and evaluation, and can comprehensively apply these skills to evaluate stories.

    # Goals:
    Based on the story content, conduct a detailed evaluation of the story focusing on the {aspect} aspect, and provide an analysis process.
...
other_instruction="""
    ...
    # Workflow:
    1. Output 【Content Analysis】: Carefully read the **Story Content** and **Comment Aspect** from the **Input Section**. Understand the **Story Content** and analyze the story's existing issues.
...
if __name__ == "__main__":
    ...
    output_base_dir = "../StoryER"
    
    model_type = "gemini-2.5-pro-06-17"  # Can be changed to other models
    temperature = 0.6
    top_p = 0.95
    ...
    try:
        ...
        for idx, sample in enumerate(data):
            ...
            try:
                ...
                # Ensure instruction variable is defined, if not use simple format
                try:
                    input_content = instruction.format(context=context, aspect=aspect)
                except NameError:
                    # If instruction is not defined, use simple format
                ...
                try:
                    ...
                    # Merge all chunks into complete response
                ...
                # Save every 10 samples to prevent data loss
                if (idx + 1) % 10 == 0:
